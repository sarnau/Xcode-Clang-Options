...
with open('Xcode Compiler Options.md', 'w') as f:
	for plistComponent in plistlib.load(open(tempSpecFile,"rb")):
		if 'InputFileTypes' not in plistComponent: # no assigned filetypes => we can ignore that
			continue
...
os.system('open "Xcode Compiler Options.md"')
#os.system('bbedit "%s"' % (xcodePluginsResourcesPath + "Clang LLVM 1.0.xcspec"))
